...
module.exports.weapon = function(req, res) {
    Product.find({ cateProduct: 'Vape' }, function(err, data) {
        if (err) {
            res.json({ "kq": 0, "errMsg": err });
        } else {
            Session.findOne({ _id: req.session.views }, function(err, data1) {
                if (err) {
                    ...
                    res.render('./paperuser/Vape', { listproduct: data, listdata: data1 })
                }
            });
        }
    })
}
module.exports.char = function(req, res) {
    Product.find({ cateProduct: 'Juice' }, function(err, data) {
        if (err) {
            res.json({ "kq": 0, "errMsg": err });
        } else {
            Session.findOne({ _id: req.session.views }, function(err, data1) {
                if (err) {
                    ...
                    res.render('./paperuser/Juice', { listproduct: data, listdata: data1 });
                }
            })
        }
    })
